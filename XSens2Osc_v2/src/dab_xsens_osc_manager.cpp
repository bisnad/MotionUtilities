...
OscSender::send()
{
	...
	try
	{
		...
		for (auto skel_id : skeletonIds)
		{
			...
			// pos_local needs the matching world ROTATIONS to express the
			// world-space parent->child delta in the parent's rotated frame
			// (see convertWorld2Local's position overload). Fetch
			// /joint/rot_world up front, alongside /joint/pos_world.
			bool haveRotWorldForThisSkeleton = skeleton->propertyAvailable("/joint/rot_world");
			std::vector<std::array<float, 4>> jointRotationsWorldRemappedForPos;

			if (haveRotWorldForThisSkeleton)
			{
				std::vector<float> rotProperty = skeleton->property("/joint/rot_world");
				std::vector<std::array<float, 4>> jointRotationsXSensForPos(jointCount);

				for (int jI = 0, dI = 0; jI < jointCount; ++jI)
				{
					for (int d = 0; d < 4; ++d, ++dI)
					{
						jointRotationsXSensForPos[jI][d] = rotProperty[dI];
					}
				}

				jointRotationsWorldRemappedForPos = fbxManager.remapJointIndices(jointRotationsXSensForPos);
			}
			...
			for (auto properyName : propertyNames)
			{
				...
				if (properyName == "/joint/pos_world")
				{
					if (!haveRotWorldForThisSkeleton)
					{
						// Cannot compute a correct pos_local without matching
						// rotations yet -- skip this cycle's position
						// conversion rather than send garbage.
						continue;
					}
					...
					// joint index remap from mvn to fbx (stateless)
					std::vector<std::array<float, 3>> jointPositionsWorldRemapped = fbxManager.remapJointIndices(jointPositionsXSens);
					// joint positions world to local conversion -- expressed in
					// the parent's rotated frame, using this frame's matching
					// world rotations (already remapped above). Stateless: no
					// calibration involved.
					std::vector<std::array<float, 3>> jointPositionsLocalRemapped = fbxManager.convertWorld2Local(jointPositionsWorldRemapped, jointRotationsWorldRemappedForPos);
					// swap coordinates
				...
				else if (properyName == "/joint/rot_world")
				{
					...
					// joint rotation world to local conversion. No rest-pose
					// calibration -- Xsens/MVN Analyze already anatomically
					// calibrated rot_world relative to the performer's own
					// T-pose/N-pose once, at the start of the capture
					// session. Stateless: no skeleton id needed anymore.
					...
					// convert vector<array<float,4>> to vector<float>
