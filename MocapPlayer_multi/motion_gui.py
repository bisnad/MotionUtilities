...
class MotionGui(QtWidgets.QWidget):
    ...
    def __init__(self, config):
        ...
        self.max_skel_display_count = config["max_skel_display_count"]
        ...
        # header
        file_names = self.player.get_file_names()
        ...
        for file_name in file_names:
            ...
            file_label += " "
        self.q_file_label = QtWidgets.QLabel(file_label)
        ...
        skeleton_count = self.player.get_skeleton_count()
        self.skel_display_count = min(skeleton_count, self.max_skel_display_count)
        
        self.pose_canvas = gl.GLViewWidget()
        self.pose_canvas_lines_all = []
        self.pose_canvas_points_all = []
        
        for sI in range(self.max_skel_display_count):
            
            canvas_lines = gl.GLLinePlotItem()
            canvas_points = gl.GLScatterPlotItem()
            self.pose_canvas.addItem(canvas_lines)
            self.pose_canvas.addItem(canvas_points)
                                     
            self.pose_canvas_lines_all.append(canvas_lines)
            self.pose_canvas_points_all.append(canvas_points)
    ...
    def load_files(self, file_path, file_names):
        ...
        self.player.load(file_path, file_names)
        ...
        self.q_file_label.setText(file_label)
    ...
    def update_player(self):
        ...
        self.player.update()       
        self.player_pose_wpos_all =  self.player.get_poses("pos_world")
        self.player_pose_wrot_all = self.player.get_poses("rot_world")
        self.player_pose_lpos_all =  self.player.get_poses("pos_local")
        self.player_pose_lrot_all = self.player.get_poses("rot_local")
    ...
    def update_osc(self):
        ...
        for sI in range(skeleton_count):
        
            osc_values = np.reshape(self.player_pose_lpos_all[sI], (-1)).tolist()
            self.sender.send("/mocap/{}/joint/pos_local".format(sI), osc_values) 
            
            osc_values = np.reshape(self.player_pose_lrot_all[sI], (-1)).tolist()
            self.sender.send("/mocap/{}/joint/rot_local".format(sI), osc_values) 
                
            osc_values = np.reshape(self.player_pose_wpos_all[sI], (-1)).tolist()
            self.sender.send("/mocap/{}/joint/pos_world".format(sI), osc_values) 
                
            osc_values = np.reshape(self.player_pose_wrot_all[sI], (-1)).tolist()
            self.sender.send("/mocap/{}/joint/rot_world".format(sI), osc_values) 


    def update_pose_plot(self):

        for sI in range(self.skel_display_count):
            
            pose = self.player_pose_wpos_all[sI]
            pose_canvas_lines = self.pose_canvas_lines_all[sI]
            pose_canvas_points = self.pose_canvas_points_all[sI]

            points_data = pose
            lines_data = pose[np.array(self.edges).flatten()]
    
            pose_canvas_lines.setData(pos=lines_data, mode="lines", color=(1.0, 1.0, 1.0, 0.5), width=self.view_line_width)
            pose_canvas_points.setData(pos=pose, color=(1.0, 1.0, 1.0, 0.5))
            
        for sI in range(self.skel_display_count, self.max_skel_display_count):
            
            pose_canvas_lines = self.pose_canvas_lines_all[sI]
            pose_canvas_points = self.pose_canvas_points_all[sI]
            
            pose_canvas_lines.setData(pos=lines_data, mode="lines", color=(1.0, 1.0, 1.0, 0.0), width=self.view_line_width)
            pose_canvas_points.setData(pos=pose, color=(1.0, 1.0, 1.0, 0.0))
