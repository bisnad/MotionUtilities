...
class Mocap_Tools:
    ...
    def npz_to_mocap(self, npz_data, skeleton_parents=None, target_fps=None):
        ...
        # 1. Identify all unique skeleton indices by inspecting the keys
        ...
        # Helper to remove non-incremental times from the time_arrays
        ...
        # 2. Process each skeleton independently
        for skel_idx in skeleton_indices:
            prefix = f'/mocap/{skel_idx}/joint/'
            try:
                pos_world = npz_data[f'{prefix}pos_world_values']
                pos_world_time  = npz_data[f'{prefix}pos_world_timestamps']
                pos_local = npz_data[f'{prefix}pos_local_values']
                pos_local_time  = npz_data[f'{prefix}pos_local_timestamps']
                rot_world = npz_data[f'{prefix}rot_world_values']
                rot_world_time  = npz_data[f'{prefix}rot_world_timestamps']
            ...
            # Derive joint count and reshape arrays
            joint_count = pos_world.shape[1] // 3
            pos_world = np.reshape(pos_world, (pos_world.shape[0], joint_count, 3))
            pos_local = np.reshape(pos_local, (pos_local.shape[0], joint_count, 3))
            rot_world = np.reshape(rot_world, (rot_world.shape[0], joint_count, 4))
            rot_local = np.reshape(rot_local, (rot_local.shape[0], joint_count, 4))
                
            # Filter valid (incremental) timestamps
            valid_pw = get_valid_indices(pos_world_time)
            valid_pl = get_valid_indices(pos_local_time)
            valid_rw = get_valid_indices(rot_world_time)
            valid_rl = get_valid_indices(rot_local_time)

            pos_world = pos_world[valid_pw]
            pos_world_time  = pos_world_time[valid_pw]
            pos_local = pos_local[valid_pl]
            pos_local_time  = pos_local_time[valid_pl]
            rot_world = rot_world[valid_rw]
            rot_world_time  = rot_world_time[valid_rw]
            rot_local = rot_local[valid_rl]
            rot_local_time  = rot_local_time[valid_rl]

            # Dynamically calculate FPS from average dt if not strictly provided
            if target_fps is not None:
                fps = target_fps
            else:
                fps = 1.0 / np.mean(np.diff(pos_local_time)) if len(pos_local_time) > 1 else 30.0

            # Find the overlapping valid time window strictly inside all bounds for Slerp
            ...
            if start_time >= end_time:
                print(f"Skipping skeleton {skel_idx}: No overlapping time window across positional and rotational data.")
                continue
            ...
            interp_pos_world = interp1d(pos_world_time, pos_world, axis=0, kind='linear', assume_sorted=True)
            ...
            for j in range(joint_count):
                # UPDATED
                slerp_rot_world = Slerp(rot_world_time, Rotation.from_quat(rot_world[:, j, :]))
                resampled_rot_world[:, j, :] = slerp_rot_world(time_target).as_quat()
                
                # UPDATED
                slerp_rot_local = Slerp(rot_local_time, Rotation.from_quat(rot_local[:, j, :]))
            ...
            # Extract mean offsets directly from Local Positional Data
            offsets = np.zeros((joint_count, 3))
            for i in range(joint_count):
                p = parents[i]
                if p == -1:
                    if frame_count > 0:
                        offsets[i] = resampled_pos_world[0, i].copy()
                        offsets[i, [0, 2]] = 0.0 # Standardize root to ground tracking
                else:
                    if frame_count > 0:
                        offsets[i] = np.mean(resampled_pos_local[:, i, :], axis=0)
